...
def generate_full_report(review:dict, weights:dict, weighted_score:int, recommendation:str=None) -> bytes:
    pdf = BasePDF()
    pdf.add_page()
    # Use passed recommendation (score-based override) if provided
    ...
    if pal:
        ...
        for hdr,w in [("#",8),("Action Required",110),("Location",44),("Severity",18)]:
            pdf.cell(w,6,safe(hdr),border=1,fill=True)
        ...
        for i,a in enumerate(pal):
            ...
            sev=a.get("severity",""); sev_col={"CRITICAL":(220,38,38),"MAJOR":(234,88,12),"MINOR":(37,99,235)}.get(sev,(0,0,0))
    ...
    pdf.set_fill_color(30,58,138); pdf.set_text_color(255,255,255)
    for hdr,w in [("#",7),("Chapter Title",68),("Present",18),("~Pages",13),("Score",14),("Key Issues",50)]:
        pdf.cell(w,6,safe(hdr),border=1,fill=True,align="C" if w<20 else "L")
    pdf.ln(); pdf.set_text_color(0,0,0)
    ...
    for i,ch in enumerate(review.get("chapters",[])):
        ...
        pdf.cell(50,5,iss_txt,border=1,fill=True,ln=True)

    # ── Technical Elements ──
    te=review.get("technical_elements",{})
    if te:
        pdf.ln(3)
        pdf.sec("Technical Elements")
    ...
    if strengths:
        pdf.ln(2)
        pdf.sec("Strengths")
        for s in strengths: pdf.bul(str(s)[:120],color=(22,101,52))
...
def generate_report_card(review:dict, weights:dict, weighted_score:int, recommendation:str=None) -> bytes:
    ...
    pdf.set_font("Helvetica","",9)
    pdf.cell(sig1,5,"Guide:",ln=False)
    pdf.cell(sig2,5,"Project In-Charge",ln=False)
    ...
    pdf.cell(sig2,4,"Signature",ln=False)
    pdf.cell(sig3,4,"HoD Signature",ln=True)
...
def _ws(review, weights):
    ...
    dim={"format":review.get("format_compliance",{}).get("score",0),"front_matter":review.get("front_matter",{}).get("score",0),
         "chapters":ch_avg,"technical":review.get("technical_elements",{}).get("score",0),
         "abstract":review.get("abstract",{}).get("score",0),"references":review.get("references",{}).get("score",0),
